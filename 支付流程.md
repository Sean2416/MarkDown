...
- #### ISSUE
  ...
  - 查詢訂單資料及付款狀況
    ...
    - ```sql
      ...
      SELECT * FROM ms_payment_pay_response where serial_number= ms_payment_pay_history.ID;
...
1. #### 新增支付對應表
   ...
   2. SQL Server-muchnewdb(cfg_storeid)
      ...
      - ###### 不確定要不要新增，對應步驟3、4的`cfg_store_sn`

   3. MSQL - Payment (cfg_pay_platform)
      ...
      - ```SQL
        ...
        VALUES (23, 99, "香港支付", 1, 1, 275, now(), 'H', 'ThirdPayment', 'Hk.png', 'hkpay', '香港支付', 99, 1 ,0,'HK')
        ```
   ...
   4. SQLServer - muchnewdb(CfgPayPlatform)
      ...
      - ```sql
        INSERT INTO  TutorDefinition.dbo.CfgPayPlatform
             ...
             VALUES
                   (23,99,'香港支付', 1, 1, 275, GETDATE(), 'H', 'ThirdPayment', 'Hk.png', 'hkpay', '香港支付', 99, 'HK')
        ```
...
1. #### 新增支付設定
   ...
   - ```sql
     ...
     Values
     (99, 3, 'HK', '香港支付', 1, 1,'{
         "MerchantID": "MS142152180",
         "HashKey": "EEHySRC9XoMlJjoH6MRUP24ZAmAncgdl",
         "HashIV": "Cr3VyhHh8RQ4inIP",
         "GateWay": "https://ccore.spgateway.com/MPG/period"
       }')
     ```
...
- ## 取得支付渠道
  ...
  1. ##### Call ClientServiceAPI取得付款詳情
     ...
     - ###### 處理: ProcessGetPaymentDetail(website判斷)

  2. ##### 呼叫28201透過Website取得對應國家支付渠道

     - Payment.MemberCenter.28201\TutorGroup.VIPABC.MemberWeb.WebSite\Controllers\PaymentController.cs (GetPayPlatform)
     - API: `http://localhost:13178/Payment/GetPayPlatform?website=H`
     - 至SQL Server取得CfgPayPlatform資料表內WebSite = 指定國別的資料
