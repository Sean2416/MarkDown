...
| received_amount  | 收款金額           | DECIMAL(20,4) |          | 對應點擊紀錄的實際付款金額                                   |
...
- ##### 訂閱制的支付渠道 PayMode 都為152(金流可以透過這個判斷是否為訂閱)

  - ###### cfg_pay_mode = 152

  - ###### CPD.Pay_Mode = 152

  - ###### cfg_pay_platform.pay_mode_sn = 152

- ##### 合約 PayType= 8 (新訂閱制合約)

- #### 付款流程說明

  - ##### 直購頁取得產品時依據(paytype)回傳訂閱的支付渠道選項(platforms)

    - ###### 使用者按下付款時會先透過28201再轉至28203(CheckProductCanSubscribe)判斷是否有還在訂閱中的相同產品

  - ##### 使用者選擇付款後進入28202(PaymentController)進行相關驗證

    1. ###### 一般付款資訊驗證

    2. ###### 若帶入渠道為訂閱渠道(bank.PayModeSn)進入訂閱邏輯驗證

       1. ###### 透過28203取得訂閱Group 及判斷是否可以訂閱 (CheckProductCanSubscribe)

          - ###### 從MySQL subscription_event_log查詢是否還有扣款中的相同產品

          - ###### 透過OMS API 取得訂閱合約Group，`如果使用者沒辦法訂閱相同產品回傳NULL`

       2. ###### 依據前一步驟回傳的結果判斷是否可以進行試用`IsWithinTrial`及塞入訂閱產品的相關資訊(週期、扣款頻率)

       3. ###### 更新CPD內容(將PayMode更新為152)

  - ##### 導至28203處理支付渠道

    - TutorGroup.Payment.BankInterface.Stripe(Pay)

- ### 完款流程說明

  - #### Stipe 付款完成Callback處理

    - ###### 接口: StripeNotifyController.Callback

    - ###### Stripe 觸發事件: invoice.payment_succeeded

  - #### 先將訂閱事件推到Queue處理避免重複Callback

    - ##### Stripe有偶發性重複事件情況，單靠資料庫來不及判斷是否重複

    - ##### Consumer在Fst.API僅做導傳回到28203`General/ExcuteSubscriptionEvent`

  - #### 訂閱事件處理 `ExecuteSubscriptionComplete`

    - ##### 扣款失敗處理 `ExecuteFailSubscripionProcess`

      - ###### 先從subscription_event_log裡面取出最後一次試用或付款紀錄 (EventType >= 0)

      - ###### 計算付款失敗次數含這次超過三次則自動幫使用者取消(RetryNum >=3)

        - ###### 走取消訂閱流程

      - ###### 三次內則更新下次扣款日及累加錯誤次數

    - ##### 試用期完款處理 `ExecuteTrialSubscritpionProcess`

      - ###### 若Callback扣款金額=0代表試用扣款

      - ###### 透過OMS API 取得訂閱合約Group，判斷是不是能夠提供試用
