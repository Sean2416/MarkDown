...
- ```yaml
  ...
  ENV TZ="Asia/Taipei"
  #ASP.NET Core 应用程序在 Docker 容器中启动时就会监听在该端口上。
  ...
  - ```yaml
    ...
    spec:
      ...
      strategy:
        type: RollingUpdate
        rollingUpdate:
          #代表 Kubernetes 會先開好 1 個新 pod 後才刪掉一個舊的 pod，整個升級過程中最多會有 3+1 個 pod
          maxSurge: 1
          # Kubernetes 整個升級過程中最多會有 1 個 pod 處在無法服務的狀態
          maxUnavailable: 1
      selector:
        matchExpressions:
...
- ##### RollingUpdate: 逐步替換舊有的Pod直到全部更新為新版本為止。 
  ...
  - ![replica](https://raw.githubusercontent.com/Sean2416/Pic/master/img/101304.png)
...
- ```yaml
  ...
  # or
  #直接編輯Deploy 的Yaml檔案，儲存後自動更新
  > kubectl edit deployment myapp-deploy --record
  #kubectl set image deploy [deploy-name] [container name]=sean2416/test:1.1.0
  >kubectl set image deploy myapp-deploy test=sean2416/test:1.0.0
...
- ![replica](https://raw.githubusercontent.com/Sean2416/Pic/master/img/20240513001.png)
  
- ![replica](https://raw.githubusercontent.com/Sean2416/Pic/master/img/20240513002.png)


### Service

- #### 在 pod 的前方提供了一個抽象層，讓外部的服務可以用 **domain name** 的方式存取 pod

- #### 創建 Service 物件時，Kubernetes 就會預設一組virtual IP 給 Service 物件。除非我們在 Service [yaml](https://zh.wikipedia.org/wiki/YAML) 指定想要的 virtual IP，否則 Kubernetes 每次都會隨機指定一組virtual IP。

- #### Kubernetes 中用來定義「一群 Pod 要如何被連線及存取」的元件。

  - ##### 當 Service 指定存取某些特定的標籤時，[Label Selector](https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/) 會根據 Pod 身上自帶的標籤進行分組，並回傳 Service 所要求的 Pod 資訊。
  ...
  - ![Service IP](https://tachingchen.com/img/kubernetes-service-in-detail/k8s-service-ip.jpg)
...
- ##### 讓不同 namespace 中的以 ClusterIP 形式生成的 Service 可以利用 ExternalName 設定的外部名稱，藉以連到指定的 namespace 中的 Service

#### LoadBalance

- ##### 除了擁有 NodePort 可以讓外部連線的優點，同時也建立了負載平衡的機制來分散流量

  - ######  目前只有雲端服務，如：GCP、AWS等等有支援而已
...
- ###### 由 Google 發布的一個輕量級工具。讓開發者可以在本機上輕易架設一個 Kubernetes Cluster，快速上手 Kubernetes 的指令與環境。[Minikube](https://github.com/kubernetes/minikube) 會在本機上跑起一個 virtual machine，並且在這 VM 裡建立一個 signle-node Kubernetes Cluster

- https://minikube.sigs.k8s.io/docs/start/

- [Kubernetes - minikube Windows安裝](https://hackmd.io/@JYU/B1ED1Po-q)
...
- [K8s官網](https://kubernetes.io/zh-cn/docs/concepts/overview/)
