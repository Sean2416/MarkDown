...
- ##### 所有請求的唯一入口，也是 Cluster 中各個 Node 的溝通橋樑，負責身份驗證（Authentication）、授權（Authorization）、存取控制（Access Control）和 API 註冊（Registration）
...
- ##### 自動化控制中心，負責管理並運行 K8s Controller 的組件。
