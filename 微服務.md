...
- ```powershell
  ...
  docker run -e "ASPNETCORE_ENVIRONMENT=Dev" -v logs:/app/data -d -p  9000:80 --name icw icw_biosecurity_api
  ```
...
- ```powershell
  ...
  # -d: 背景執行
  # docker run  -p  -d [外部 ip]: [內部 ip] --name [容器名稱] [RepositoryName]:[Tag]
  docker run -e "ASPNETCORE_ENVIRONMENT=stage" -v /log:/app/data -d -p  8096:80 --name icw icw_biosecurity_api
  
  //透過環境變數啟動Container
  docker run -e "ASPNETCORE_ENVIRONMENT=Prod"  -t image-hub.tutorabc.com/twm/clientserviceapis:Stage-master-20231215_1252
...
docker run -e "ACCEPT_EULA=Y" -e "SA_PASSWORD=Sean0721"  -p 1433:1433 --name sql2019 -h sql2019   -d mcr.microsoft.com/mssql/server
```
