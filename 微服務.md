...
- ##### 流程說明
  ...
  - ###### 使用者（resource owner）從瀏覽器（agent）打開網頁服務（client）開始，從網站上點選登入按鈕後，會**觸發瀏覽器redirect**到authorization server，並攜帶下列內容
  ...
  - ###### 使用者在authorization server進行驗證及授權，讓服務（client）能使用服務於第一步scope參數內請求要使用的資料

  - ###### authorization server得到使用者（resource owner）的認證與授權過後，將以下auth code送去使用者當初Client所提供的endpoint（或是request時提供的redirect_uri），而endpoint以最佳實踐來說應該要是個後端程式(先導到後端進行處理，再轉回前端)
  ...
  - ###### 後**端程式發出一個POST request到authorization server**並攜帶第4步驟取得的authorization code和client_id，authorization server在利用authorization code驗證過使用者後，會將access token和其他欄位（如下所示）一同傳回
  ...
  - ###### 之後就可以利用access token來存取位於resource server的資料，由於安全性因素，建議不要將access token以明碼（plain-text）的形式傳到前端去，若要存取resource server最好都還是由後端進行

#### Implicit Grant

- ###### 跟Authorization Code Grant最大的差異，由圖發現是Implicit完全沒有backend server的參與，完全是由browser中的Fronend Application作為OAuth的client。因此Implicit Grant也比較適合Single Page Application使用

  - ![img](https://miro.medium.com/v2/resize:fit:700/0*asIJwovqFHWNPgBF)

- ###### Frontend Application（client）觸發request並導（Redirect）往Authorization Server進行驗證和授權
...
- ###### 透過browser（user-agent）向使用者（resource owner）索取，他們於Authorization Server中所使用的帳號密碼進行**驗證**，並根據frontend application要求的scope，向使用者要求授權

- ###### 驗證與授權過後，會透過導轉（redirect）將access token置於parameter中回應給使用者
...
- ###### web app可以透過access token去向resource server請求所需要的資料
...
- ##### User登入驗證成功後，轉回Client 並且帶上GrandCode
...
- [使用Line Login實現oAuth SSO](https://studyhost.blogspot.com/2016/12/linebot7-line-loginoauth-sso.html)
...
- [[ASP.NET Identity 如何取得 Facebook 登入的 Access Token](https://blog.miniasp.com/post/2014/04/13/ASPNET-Identity-Get-Access-Token-Facebook)](https://blog.miniasp.com/post/2014/04/13/ASPNET-Identity-Get-Access-Token-Facebook)
