...
- ```yaml
  ...
  # 使用 dotnet build 命令建置應用程式
  RUN dotnet build "./TEST.csproj" -c $BUILD_CONFIGURATION -o /app/build
  
  # 發佈階段，從建置階段中繼承
  FROM build AS publish
  
  # 再次定義建置配置（預設為 Release）
  ARG BUILD_CONFIGURATION=Release
  
  # 使用 dotnet publish 命令發佈應用程式
...
- ##### Kubernetes 的 Pods 調度員，scheduler 會監視新建立但還沒有被指定要跑在哪個 Node 上的 Pod，並根據每個 Node 上面資源規定、硬體限制等條件去協調出一個最適合放置的 Node 讓該 Pod 跑

- #####  K8s 的工作調度器，它負責監控所有使用者開啓 Pod 的指令，並根據 Worker Node 的資源規定和硬體限制找出最合適的 Worker Node。
...
- ##### Kubernetes 是如何建立一個 Pod？

  1. ###### 使用者要先透過 User Command（kubectl）輸入建立 Pod 的對應指令。此時指令會經過一層確認使用者身份的認證後，傳遞到 Master Node 中的 API Server，API Server 會把指令備份到 etcd 。

  2. ###### controller-manager 會從 API Server 收到需要創建一個新的 Pod 的訊息，並檢查如果資源許可，就會建立一個新的 Pod。

  3. ######  Scheduler 在定期訪問 API Server 時，會詢問 controller-manager 是否有建置新的 Pod，如果發現新建立的 Pod 時，Scheduler 就會負責把 Pod 配送到最適合的一個 Node 上面。



## Worker Node

- ##### K8s 中的工作主機，負責管理和運行 Pod。它可以是實體機，也可以是虛擬機（例如：AWS 上的 EC2）。

- ![Worker Node](https://www.omniwaresoft.com.tw/wp-content/uploads/2024/02/%E6%88%AA%E5%9C%96-2024-02-07-%E4%B8%8B%E5%8D%882.57.16.png)

### Kubelet

- ##### Node 的管理員，負責管理該 Node 上的所有 Pods 的狀態並負責與 Master 溝通

- ##### Worker Node 與 Kube-API Server 進行溝通的元件， 負責接收 Pods 的當前狀態以檢查當中的容器健康狀態
...
- ##### Node 的傳訊員，負責更新 Node 的 iptables，讓 Kubernetes 中不在該 Node 的其他物件可以得知該 Node 上所有 Pods 的最新狀態

- ##### 每個 Node 上運行的網路代理服務，它負責管理 Pod 間的網路通信規則、 Cluster 內部的通訊與回應 Cluster 外部的 request 。如果作業系統中存在封包過濾器（packet filtering layer ），Kube-proxy 會將處理 request 的請求轉由 Worker Node 的作業系統處理。 

### Container Runtime

- ##### Node 真正負責容器執行的程式，以 Docker 容器為例其對應的 Container Runtime 就是 Docker Engine

- ##### 屬於較為底層的元件，負責實際運行容器，並聽從 Kubelet 的命令管理容器。K8s 支援多種不同的 Container Runtime，例如 containerd 、 runC 、 CRI-O 等。
...
- ##### 檢視Pod 的狀態訊息
  ...
  - ![replica](https://raw.githubusercontent.com/Sean2416/Pic/master/img/pod_describe.png)
...
- ##### yaml file
  ...
  - ##### 將Pod對應本機Port
    ...
    - ###### 5000外部對應的port，8080為Pod內部揭露的port
...
- #### Pod Hook
  ...
  - #### PostStart
    ...
    - ##### 範例
      ...
      - ###### 啟動一個 nginx 容器, 執行一段指定的 command

      - ###### 將 **Hello this is post-start-demo-hook-handler** 這一段話 echo 到 **/demo-messages** 檔案
  ...
  - ##### PreStop
    ...
    - ##### 範例
      ...
      - ###### 將 **Hello this is the pre-stop-demo-hook-handler** 這段 string echo 到 /demo/message 這個檔案中

        - ###### 因為這個 pod 馬上就要關閉了, 如果關閉了自然 pod 內的 `/demo/message` 也會銷毀, 所以我們在外頭定義一個 volume, 並掛在到容器裡, 這樣當 string 被 echo 到 `/demo/message` 時, 也會同步到外面來

      - ###### 執行 `cat var/tmp/demo/message`, 成功印出 **Hello this is the pre-stop-demo-hook-handler**, 代表在容器結束前, 確實有執行 preStop handler
  ...
  - #### Hook handler 實作
    ...
    - ##### Exec 
      ...
      - ######  在 cgroups 以及命名空間中, 執行一個特定的指令, 像是 `pre-stop.sh`。 這個指令消耗的資源會被算在這個容器上

    - ##### HTTP 

      - ###### 在容器的一個特定端點執行一個 HTTP 請求
...
- ##### Pod創建流程
  ...
  - ##### 說明
    ...
    4. ###### K8s上的組件都會使用`watch`機制來追蹤檢查API Server的狀態變化
  ...
  - ![https://ithelp.ithome.com.tw/upload/images/20200913/20129656XdwQzeTUJb.png](https://ithelp.ithome.com.tw/upload/images/20200913/20129656XdwQzeTUJb.png)

- #### Pod刪除流程

  1. ###### 用戶提交刪除Pod指令, 預設的寬限期(grace period)是 30s

     - 可以透過`kubectl delete --grace-period=<seconds>`改變預設值, 若是要設定為0則要搭配 `--force`, 設定0表示直接強制刪除指定的Pod。

  2. ###### API Server中的Pod被更新, 如果再寬限期內未完成, API Server內的Pod狀態會被更新為`dead`
  ...
  7. ![https://ithelp.ithome.com.tw/upload/images/20200914/20129656CjGOpqAhog.png](https://ithelp.ithome.com.tw/upload/images/20200914/20129656CjGOpqAhog.png)


#### ~~Replication Controller (使用ReplicaSet 取代)~~

- ##### 主要的目的是確保K8S上目標Pod的數量符合設定值
...
- ###### yaml file
  ...
  - ```yaml
    ...
    metadata:
      name: nginx
    spec:
      #代表這個ReplicationController要把這個Pod的數量維持在多少
      replicas: 3
      #代表這個ReplicationController會監控哪個label，像是這邊的範例就是”app: nginx”，剛好與template內的labels是相同的，所以template的Pod會被維持在3個。
      selector:
        app: nginx
      #用來補足Pod數量時所用的Pod模板
      #內容就和Pod的metadata與spec一樣。
      template:
        metadata:
          name: nginx
          labels:
...
- #### 與Replication Controller有相同的功能，但提供更多的幫助

  - ##### **Selector** 

    - ###### Replication Controller是以完全相等作為判斷

    - ###### Replica Set可以提供更彈性的判斷條件
...
- ##### Yaml 
  ...
  - ```yaml
    ...
    spec:
      ...
      replicas: 3
      selector:
        #監控符合Label的Pod，如果原本就有Pod符合下面的條件，執行replicaset時就不會再多新增，總數會與設定數量相同
        matchLabels:
          type: front-end
        #支持
        matchExpressions:
...
- ##### 如果原本就有相同設定的Pod存在(符合selector條件)，則執行ReplicaSet時會算進去要開的replica數量(刪除時也會把符合條件的一併刪掉)

  - ![replica](https://raw.githubusercontent.com/Sean2416/Pic/master/img/203945.png)
...
- 動態擴展Pod數量
  ...
  - `kubectl scale --replicas=6 -f test.yaml` or `kubectl scale --replicas=5 replicaset myapp-replicaset`
  - ![replica](https://raw.githubusercontent.com/Sean2416/Pic/master/img/204749.png)



#### Deployment 

- #### 無狀態的水平擴展控制器,主要功能也是確保Pod能正常運作, 它建構在Pod和ReplicaSets之上, 可以為Pod和ReplicaSets提供聲明式更新, 大部分的功能都可以透過調用ReplicaSet Controller來實現。

- ##### Deployment大部分都和ReplicaSet相同, 最大的不同在於 deployment支援滾動更新, 這也是為什麼deployment是被實踐最多的Controller。
...
- #### 創建 Deployment
  ...
  - ##### 由於它建構在ReplicaSet之上, 所以spec中的字段包含ReplicaSet Controller的`replicas`, `selector`, `template`, `minReadySeconds`, 他也是利用這些字段內容完成了下一層的ReplicaSet創建。
...
-  kubelet 使用 startup probe 來決定一個容器應用是否已經成功啟動了。

- ###### kubelet 使用 liveness probe 來決定何時重啟容器, 舉例來說, 當你的應用處於運行中, 但是卻無法處理 request。

- ###### kubelet 使用 readiness probe 來決定這個容器是否已經準備就緒, 可以接收流量了。 當一個 Pod 內的所有容器都就緒了, 該 Pod 才算是就緒。
...
- ###### 啟動時監測設定的health check路徑是否可以連線

###### livenessProbe

- ###### 定時檢查運行中的Pod是不是都有正常在工作。用來預防Pod長時間 運行後，裡面出了問題卻沒被發現。通常用於自我檢查，不會去測試與其他服務的連線

- ![replica](https://raw.githubusercontent.com/Sean2416/Pic/master/img/k8s_livenessProbe_Fail.png)

###### readinessProbe

- ###### 監控第三方服務(資料庫、外部API)是否正常運行

- ![replica](https://raw.githubusercontent.com/Sean2416/Pic/master/img/k8s_readinessProbe_Fail.png)



##### 更新策略

- ##### recreate: 全面停止、刪除舊的Pod之後以新版本重建。

  - ###### 這也是ReplicaSet的更新方式, 他會導致新舊版本更新期間服務中斷, 優點是服務不會有中間狀態, 不是新版本就是舊版本。

- ##### RollingUpdate: 逐步替換舊有的Pod直到全部更新為新版本為止。 

  - ###### Deployment更新策略 `default` 為滾動更新, 在刪除舊版本的同時也在創建新版本, 優點是服務不中斷, 缺點是在更新期間執行的請求不保證會是新版本。

  - ![https://ithelp.ithome.com.tw/upload/images/20200915/20129656wn25Hvn6Y3.png](https://ithelp.ithome.com.tw/upload/images/20200915/20129656wn25Hvn6Y3.png)
...
- ```yaml
  ...
  # or
  #kubectl set image deploy [deploy-name] [container name]=sean2416/test:1.1.0
  > kubectl set image deploy myapp-deploy tes=sean2416/test:1.1.0
  
  # 檢視 Rollout
  > kubectl rollout status myapp-deploy
  > kubectl rollout history myapp-deploy
...
- #### Kubernetes 中用來定義「一群 Pod 要如何被連線及存取」的元件。
  ...
  - ```yaml
    ...
    spec:
      ...
      ports:
        - protocol: TCP
          # 指定的 Pod 的 port number
          targetPort: 80
          #設定Pod 在Service上暴露的接口，類似Run Container時的 8081:80。沒有對外開放
          port: 8001
          # 設定外部可以呼叫的接口
          nodePort: 30000
    ```
...
- ##### 用來永久性保存資料在實體位置上，讓資料不會因為Pod生命週期而造成遺失

- ##### K8S Volume架構

  - ##### PV透過設定對應到實體的儲存位置 >> Storage Class 綁定相同集群的PV >> PVC 從指定的Storage Class中找到符合的PV進行綁定 >> Pod 中的Container 透過Mount的方式對應到指定的PVC

  - ![replica](https://raw.githubusercontent.com/Sean2416/Pic/master/img/pod_pvc02.png)

- #### Persistent Volume，定義物理機上的儲存體設定資訊，以及 `PVC`，Persistent Volume Claim，定義儲存的需求（如容量等等），兩份設定檔分別由管理員和開發者維護，Container 的部分只需要呼叫需要的 PVC 不需要去思考實體路徑的問題，PVC 便會找到適合的 PV 作為儲存體。

  - ##### PV：描述一個具體 volume 的屬性、掛載目錄、遠端儲存體位址等等的文件

  - ##### PVC：描述 pod 需要的持久化屬性，如存儲大小、讀寫權限等等的文件

  - ##### Container mount到對應的PVC <>  PVC依據設定的條件對應到合適的PV <>  PV將資料保存到對應的實體路徑
  ...
  - ```yaml
    ...
    metadata:
      name: app-pv
    spec:
      #指定Storage Class，後續會依據Storage Class建立關聯
  ...
  - ```yaml
    ...
    metadata:
      name: app-pvc
    spec:
      #指定Storage Class要跟想要Bind的PV設定相同
      storageClassName: sc-001
      accessModes:
    	- ReadWriteMany
      resources:
  ...
  - ![replica](https://raw.githubusercontent.com/Sean2416/Pic/master/img/pvc001.png)

  - ![replica](https://raw.githubusercontent.com/Sean2416/Pic/master/img/pvc002.png)

  - ##### 如果建立的PVC沒有對應到PV的話，Status會停留在Pending

  - ![replica](https://raw.githubusercontent.com/Sean2416/Pic/master/img/pvc003.png)

  - ##### 直到能夠綁定對應的PV以後，狀態才會變更

  - ![replica](https://raw.githubusercontent.com/Sean2416/Pic/master/img/pvc004.png)

### Persistent Volume (PV)

- ##### 用來管理 cluster 中 storage 的資源，支援像是 NFS、Glusterfs、HostPath等多種存儲插件，擁有獨立的生命週期。

- #### Lifecycle

  - #### Provisioning

    - ##### PV 創建的階段，有靜態和動態兩種方式，最後的結果就是 PV 和 PVC 成功配對

    - ##### 靜態模式: 叢集管理員預先創建許多PV，PVC與這些預先創建的PV進行配對

    - ##### 動態模式: 不需要預先建立 PVC ，當使用者透過建立 PVC 向儲存類型進行申請時，系統會透過 StorageClass 的設定內容配置相對應的 PV，並與 PVC 綁定
  ...
  - #### Releasing
    ...
    - ##### 當儲存資源使用完畢之後，即可刪除 PVC，與其綁定的 PV 狀態會被轉為 `Released`，因資料內容可能還需要進行處理，待處理完成後才可以再和其他PVC進行綁定

    - ##### 管理員也可以設定資料處理的政策，有`Retain`、`Delete`、`Recycle`三個選項

      - ###### Retain：保留資料，刪除 PVC 之後，與之綁定的PV不會被刪除，僅會標記為`Released`，需管理員手動刪除資料後此PV才可以給其他PVC使用

      - ###### Delete：即刪除資料，但此參數需看雲端服務商是否支援

      - ###### Recycle（棄用）：執行`rm -rf /path/to/volume`
...
- ### Type
  ...
  - #### emptyDir
    ...
    - ##### 當Pod調度某給`Node`時，首先創建`emptyDir Volume`，並且該`Pod`在該`Node`上運行時就存在。顧名思義，它最初是空的。`Pod`中的容器都可以在`emptyDir Volume`中讀取和寫入相同的文件，儘管該`Volume`可以安裝在每個Container中的相同或不同路徑上。當`Pod`從`Node`中被刪除時，`emptyDir`中的數據也將被刪除。


- #### hostPath

  - ##### 將`Node`檔案系統中的檔案或目錄mount到`Pod`中。`hostPath` 的生命週期與 `Node` 相同，並不會隨著 `Pod`消失而消失。

    - ##### 通常這不是一般 pod 會用的 volume type，只有在一些情況下例外，例如：

      - ###### 容器應用程式產生的紀錄檔需要永久儲存時，可以使用`Node`的檔案系統進行儲存

      - ###### 需要存取`Node`上`Docker`內部資料結構的容器應用時，可以透過定義`hostPath`為`Node` 的 /var/lib/docker 目錄，使容器可以直接存取`Docker`的檔案系統
...
- #### local
  ...
  - ##### 主要用來表示在 host 上掛載的 local storage device，例如：disk, partition, directory … 等等。
    ...
    - ##### 相較於 hostPath，`local` volume type 有以下幾點的不同：
      ...
      - ###### 若 application 只能使用 local volume 的話，那就要考量到資料無法正常存取時的容錯設計

      - ###### local volume 是由 Storageclass 所產生，透過 `volumeBindingMode: WaitForFirstConsumer` 的設定，讓 PV 會在 PVC 需求實際發生(可能透過 node selector, pod affinity, pod anti-affinity … 等設定)時才會產生。
...
- ##### 依據設定的條件(容量大小...)找尋合適的PV進行綁定

- ##### PVC 是針對 PV 請求特定大小、StorageClass等等，PVC 是屬於 Namespace 層級的資源。

- ![replica](https://raw.githubusercontent.com/Sean2416/Pic/master/img/pod_pvc.png)



### StorageClass

- ##### 提供 PVC 對應儲存服務供應商的資訊和儲存空間的管理方式。

- ##### 對於儲存資源的抽象定義，為管理員提供一種描述存儲類別 Class的方法，對不同的儲存類別，配置不同的儲存體設置

- ##### 對於動態配置的策略，K8s 會直接依據 StorageClass設置內容進行PV的配置，而靜態配置則是會在可用的PV已耗盡之後才會依據StoageClass進行配置

- 

- ##### 管理員可以為PVC指定默認的 StorageClass。
...
- ##### 存放機密性的資料，而 secret volume 有以下幾個特點：
...
- ##### 適合用來存放**「非敏感性」**的未加密設定資訊



## NameSpace

- ##### K8s 利用Namespaces實現**資源隔離**的方式，不同的`namespaces`的物件無法"直接"互相存取。

  - ##### 透過`namespace`，邏輯上將不同專案、團隊或使用者分群

- ##### 在同一個 Kubernetes Cluster 中，每個 Namespaces的名稱都是要**獨特的**

- ##### 當一個Namespaces]除時，在該 Namespace 裡的所有物件也會被刪除

- ##### 可作用的元件

  - ###### Pod、Deployment、ReplicaSet、Service、Igness、PVC
...
- ##### Ingress可以統一對外的接口(80)，後續再透過不同的規則找到對應的Service
  ...
  - ##### 將不同路徑的請求對應到不同的 Service 物件
    ...
    - ##### pathType設置為Prefix，表示將請求路徑由到以/service-a或/service-b打頭的所有路徑方向。

    - ##### 如果pathType設置為Exact，需要完全一致才行，包含大小寫也要一致，只可以請求請求路徑由到 /service-a 或 /service-b 路徑

    - ```yaml
      # 將example.com/service-a 的請求路徑命名為 service-a 的 Kubernetes Service，
      # 將example.com/service-b 的請求路徑命名為service-b 的 Kubernetes 服務。
...
- ##### Ingress 定義了應用程式的路由規則，Ingress Controller是實現Ingress規則並將流量路由到不同應用程序的應用程序

- ##### **Ingress Controller 會自動監聽 Kubernetes 集合中的 Ingress 資源，並根據其中規定的規則來由請求**。
...
- [教學影片](https://www.udemy.com/course/deploying-net-microservices-with-k8s-aks-and-azure-devops/learn/lecture/24331544?start=90#overview)

- [Source Code](https://github.com/aspnetrun/run-devops)

- #### 系統架構

  - ![replica](https://raw.githubusercontent.com/Sean2416/Pic/master/img/azure_practice001.png)
  - 
  ...
  spec:
    ...
    template:
      ...
      spec:
        containers:
          - name: shoppingclient
            image: sean2416/shoppingclient:latest
            imagePullPolicy: IfNotPresent
            ports:
              # Docker File 裡面設定的Expose Port
              - containerPort: 80
            env:
  ...
  spec:
    ...
    template:
      ...
      spec:
        containers:
          - name: shoppingapi
            image: sean2416/shoppingapi:latest
            imagePullPolicy: IfNotPresent
            ports:
              - containerPort: 80
            env:
...
- ```yaml
  ...
  metadata:
    name: mongo-configmap
  data:
    connection_string: mongodb://sa1234:sa1234@mongo-service:27017
  ```
...
- ```yaml
  ...
  type: Opaque
  data:
    mongo-root-username: c2ExMjM0
    mongo-root-password: c2ExMjM0
    # mongo-root-username, mongo-root-password 是用 sa1234 做 base64 編碼做出來的字串
    # Opaque 是用來儲存 key value 對應的資料形式
  ```
...
- [Kubernetes 基礎教學](https://chengweihu.com/kubernetes-tutorial-1-pod-node/#Kubernetes-%E5%9B%9B%E5%85%83%E4%BB%B6)
...
- ##### 建立一個空白檔案，果檔案已經建立了，touch會將檔案的最後更新時間變為現在時間

  - `touch abc.txt`

## CP(Copy)

- ##### 複製檔案或資料夾，`cp 原本的檔名 新的檔名`
...
- #####  移動檔案與資料夾，也能拿來更改檔案與資料夾的名稱

- ##### 移動的指令：`mv 檔案名稱 新位置的資料夾名稱`，修改檔名的指令：`mv 檔案名稱 新的檔案名稱`。

- | 狀況       | 參數1  | 參數2  | 說明                                     |
  | ---------- | ------ | ------ | ---------------------------------------- |
  | 移動資料夾 | 資料夾 | 資料夾 | 將參數1（資料夾）放到參數2（資料夾）裡面 |
  |            | 資料夾 | 檔案   | 操作無效                                 |
  | 更名資料夾 | 資料夾 | 不存在 | 將參數1（資料夾）更改名稱為參數2         |
  | 移動檔案   | 檔案   | 資料夾 | 將參數1（檔案）放到參數2（資料夾）裡面   |
  | 覆蓋檔案   | 檔案   | 檔案   | 將參數1（檔案）覆蓋掉參數2（檔案）       |
  | 更名檔案   | 檔案   | 不存在 | 將參數1（檔案）更改名稱為參數2           |
...
- ##### `.` 一個點就是現在位置的別稱，因此輸入`cd .`之後，什麼也不會改變。

- ##### `..` 兩個點是上一層位置的別稱，輸入`cd ..`之後就會一到上一層。注意上一層與上一頁是不同的。

- ##### `~` 波折號代表**家目錄**，Windows上也有一個使用者家目錄，底下有音樂、下載、文件…的那個資料夾。

- ##### `/` 斜線代表**根目錄**，在Windows上就像你點開C槽或D槽看到的資料夾，但Linux只有一個根目錄。
...
- ##### 更改系統日期或時間（要更改系統時間需要[使用 `sudo` 或 `su` 取的 root 權限](https://blog.gtwang.org/linux/sudo-su-command-tutorial-examples/)）：

  - ```SH
    #定為 2016/11/11 早上 10:21:32
...
- ##### 刪除檔案與資料夾。被rm指令刪掉的檔案不會進到垃圾桶、資源回收桶之類的地方，非常難找回來，要小心使用。
...
- ##### 如果要將資料夾與裡面的所有東西都一起刪除，需要使用上面提到的`rm -rf 路徑`指令。
...
- ##### 也可以加上-n來指定要顯示多少行，例如顯示20行：`head -n 20 檔案名稱`。

## tail

- ##### 將檔案的最後10行顯示出來

  - ###### 例如一些日誌檔案都是越下面的越新，使用tail就會很方便的直接看到最新到日誌是什麼。我們可以用`tail -n 20 檔案名稱`來指定要看最後20行文字。

- ##### 持續輸出檔案內的最新內容

  - ###### 例如有一個日誌檔案叫做`log.txt`，他每隔幾秒就會多一行新的資料在裡面，我們只需要輸入一次`tail -f log.txt`來持續監控log.txt檔案的變化，每當log.txt多一行資料，tail就會自動顯示一行資料，一直到我們在鍵盤按下`Ctrl + C`停止它，是非常方便的功能。
...
#將Linux 指令date的輸出結果寫入 word.txt裡面
#date為系統時間指令20240425
$ echo $(date +%Y%m%d) >>  word.txt
```
...
- ### 篩選出含有關鍵字的檔案名稱
  ...
  - ```SH
    # 當前目錄下取得檔案名稱內包含"conf"的
    $ ls ./ | grep conf
    
    ClientServiceAPIs.dll.config
    ClientServiceAPIs.runtimeconfig.json
    app.config
    nlog.config
    web.config
    ```
...
- ### 不分大小寫 `-i`
  ...
  - ```SH
    $ grep NAME web.config
     #
    $ grep -i NAME web.config
...
- ### 顏色標示
  ...
  - ##### 將成功匹配的部分文字標示出來， `--color=never`、`--color=always`、`--color=auto`

  - ```shell
    $ grep --color=always -n name web.config
    ```
...
- ### 尋找檔案/尋找資料夾
  ...
  - ```sh
    ...
    # 找到當前目錄下config的檔案，並複製到config資料夾
    $ find ./ -name "*config*" -type f | xargs -i cp {} config
    
    # 找到當前目錄下config的檔案，並移動到前一個目錄下的test資料夾
    $ find ./ -name "*config*" -type f | xargs -i mv {} ../test 
    ```
...
- ### 根據檔案修改日期來找
  ...
  - ```SH
    #尋找當前目錄下在 5 分鐘以內變動過的所有檔案
    $ find ./ -type f -mmin -5
    
    #尋找當前目錄下在30天以內變動過的所有檔案，複製到test目錄
    $ find ./ -type f -mtime -30| xargs -i cp {} test
...
- ##### 直接開啟一個純文字的文字編輯器來編寫文字檔案，vim比較複雜，Ubuntu裡面甚至預設沒有安裝完整版的vim，建議可以從nano開始學習。

- ##### `nano 檔案名稱`，如果檔案名稱是已存在的就會編輯舊檔案，如果檔案名稱不存在就會建立新檔案。

  - ###### 編輯完成按下`Ctrl + X`就會問你要不要存擋，輸入Y或是N按下Enter即可。如果你回答要存擋，他就會問你要不要修改檔案名稱，這邊什麼都不打直接按下Enter即可完成。

## 帳號相關

### sudo

- ##### 使用root權限（系統最高權限）執行命令，使用方式是在指令最前面加上sudo，例如`sudo mkdir abc`。
...
- ##### 從**初始群組**（Initial group）切換**有效群組**（Effective group）。

  - ###### 一個帳號會存在一個初始群組，剛登入的時候就會在這個群組內。但同時帳號也能所屬於其他群組，稱作有效群組，可以透過newgrp指令切換到其他身份。切換後使用touch、mkdir之類的指令時，建立的檔案與資料夾的身份就會不太一樣。
...
- [鳥哥私房菜](https://linux.vbird.org/linux_basic_train/rockylinux9/unit01.php)
...
- 當初選擇共用開發框架的原因及評估
  - 當時在規劃共用開發框架的時候主要是希望可以導入分層式架構的概念，主要的目的是希望降低系統的耦合度提升偕同開發的效率及降低後面的維運成本。另外，可以將業務邏輯單獨抽離出來增加系統的彈性、確保業務邏輯的完整性跟一致性。因此，當初是以這個需求去研究線上的各個開發框架。後來我們導入了一個叫ABP的開源架構。會選擇ABP的主要原因是因為他已DDD的概念進行分層式架構的規劃並且詳細的撰寫了開發文件來規範每一層的權責範圍，這符合我們公司所希望的同一架構及寫作模式的發展策略。另外，ABP也提供了很完整的輔助，像是針對一些開發上常用的功能進行封裝(如，ORM裡面的一些共用涵式處理、系統開發上常見的使用者管理跟驗證授權...等)可以有效的降低工程師的開發成本並保留了高度的彈性。另外，他也引用了一些.NET常用的套建功能像式Hangfire、Mail發送...等。因此，透過這個開發框架可以大幅降低工程師日常開發工作並且易於維護及擴充。
- 希望未來的主管是什麼類型的人
  - 在我目前工作的歷程中，碰到的主管大多都是抱持著開放心態 鼓勵員工可以分享自己的想法及意見 並進行討論的人。有些時候我們對於事情的看法可能因為對於領域的不熟悉或不了解公司的政策因此跟主管的想法不同。透過溝通討論的方式可以讓我們對於接到的任務可以有更深入的了解，對於後續進行系統設計或是開發上也都會有很大的幫助。另外，在與主管討論的過程中也能夠提升我們的思維能力及思考面向。當然，我也明白不同的主管可能有不同的風格和價值觀，我願意與任何類型的主管合作，以達成共同的目標和任務。
- 如果你在與主管討論的過程中，發生意見不同的狀況你會怎麼處理
  - 首先，我認為在執行任務的過程中每個人的出發點都是希望可以解決某一項任務協助公司長久發展，只是對於業務的理解或公司的策略有不同的看法才會導致意見不同的情形。因此，我會主動說明我對這件事情的看法及顧慮 並且聆聽主管的想法。有些時候可能是因為我自己對於業務沒有全盤的認知或不理解公司的發展策略導致思考沒有這麼全面，所以透過這樣理性的討論下可以讓我更熟悉整個任務的細節。當然，每個主管的風格不同 主管也沒有必要想我做過多的說明，因此 在我闡述我的觀點及接收主管的想法後 我還是會以主管的考量為主軸。因為我認為主管不管是在經驗上 或對於公司、業務的理解程度都會比我還要高。所以 我還是會依照主管的意見進行規劃及開發的工作，但我會適度地在系統內針對我有疑慮的部分保留彈性以便後面如果有調整的時候能夠快速地置換。

- 為什麼會選擇百度地圖
  - 會使用百度地圖的原因 主要是因為他有提供虛擬圍欄的功能可以協助我們進行隔離監控的目的，另外 因為我們公司的主場是在大陸 所以考量到外在環境因素而使用百度地圖爾且百度地圖也可以在台灣使用，所以對我們來說是最符合需求的。
  - 當初再導入GCM的時候 主要是因為POC階段是在台灣進行測試，Google的服務相對來說比較穩定及常用。針對大陸的部份，我們改以SignalR來做為平台的通知，APP部分就改由IOS及Android各自實現。
- Oncall問題
  - 首先，我想說明一下我的狀況。因為公司金流交易模組的負責人 所以對於整個業務邏輯及系統流程部分我是最為熟悉的。另外，由於我們界接了多個支付渠道，而後台的帳密又是屬於高度隱密性的資料所以僅有少數幾個人擁有。因此，當客戶有交易問題反饋是，營運單位都會直接尋求我的協助。主要原因也是我能夠比其他人更快的找到問題並解決。所以再之前的經驗中，只要我當下能夠協助幫忙的時候 我都會立即上線處理。除非，我當時手邊沒有電腦可以操作才會協請同事幫忙。但是，同事再處理這些狀況的速度沒有我來的熟悉 所以有時候會導致客戶最終不願意付款造成訂單流失。因此，目前主要都還是我會盡力協助公司解決這方面的問題。所以我常常再下班時間或假日要隨時注意手機的訊息，有時候甚至出門要帶著電腦再身上避免突發狀況發生。
- 你認為資深工程師應該有什麼特質
  - 針對這個問題，我會分成幾個部分來回答。首先，一個資深的工程師應該要具備一定的技術能力及開發經驗。當碰到需求或問題是能夠快速做出反應並且要能夠針對不同的狀況情境給予最合適的處理方式。相比於比較資淺的工程師來說，資深的工程師擁有更充足的經驗及理解更多的技術知識。因此，再碰到一個需求時他不應該只是考量如何解決這個問題，而是要因應不同的情境給予最佳的解答(舉跨場轉移的例子。)。
  - 其次是要有獨立思考的能力，當一個資深工程師再接到需求的時候 除了要關注任務本身以外還要考量到其他面向的問題，例如跟原有的系統流程或邏輯是否有衝突。例如，當我接到要開發訂閱至交易模式的時候我會先評估整個交易流程跟現在的設計及架構是否有衝突會不會導致原有金流系統的異常。因此，當初在導入時我從產品、合約、金流三大模組進行一次流程與結構性的盤點。發現導入以後與原本的合約流程不一致也無法再現有的金流模組裡面實現，因此，我針對產品、合約、金流模組進行了重新的規劃及系統架構的調整，再不影響原有系統的狀況下重新規畫整個交易模式。另外，因為我所負責的模組牽涉到很多不同的業務單位，因此 再接到任務時除了考量功能的可行性外，也會去評估是否會影響到其他業務單位。例如，財務在對帳上會不會因為不同的產品類型及交易模式而有影響。
  - 再來，相較於資遣的工程師我能夠承擔更多的責任。除了可以進行功能開發以外也能夠進行需求分析、設計系統流程或架構也能協助檢視系統的流程及架構是否會有衝突。另外針對經手過的系統也能夠進一步的進行重構及優化來提升系統的穩定性及效能。因為技術不斷的再更新，所以要適時地進行重購及調整才能讓系統變得更好。
  - 最後，每個工程師都應該不斷的去學習。呼應前面所提到的，因為技術一直不斷的更新，所以必須不斷累積自己的技術能力才能再適當的時候提出最合適的處理方式來協助團隊解決問題。我自己是有加入一些技術社團，針對大家分享的議題我會去進行了解。另外，我也有在訂閱一些技術相關的課程來加強自己的開發及架構上的能力。像是最近為了進行店商平台的開發，就有買一些線上關於相關架構規劃的教學影片來了解目前常用的一些開發架構及技術，避免自己規劃的系統與目前主流的架構脫節。
- 你對於未來職業發展有什麼計劃或目標？
  -  身為一個資深工程師，我認為在於技術這方面必須要不斷的精進自己的能力。除了要持續吸收新的知識外，也必須要對於程式碼及系統的架構不段的優話。例如，我自己在空閒的時間會閱讀一些技術性的文章，或是對於我想了解的技術內容會購買一些線上教程來進行學習。像是，我最近因為要進行電商平台的流程設計及架構規劃，因此我購買了跟電商架構及技術的教學課程來進行學習。透過這樣的方式來不段增進自己的能力。在程式碼及架構的部分，每當專案開發到一個階段，我都會不段回頭檢視以往撰寫過的程式碼並且進行優化及重購來提升系統的可維護性及效能。另外，我希望自己可以增進談話的技巧及需求訪談的經驗，讓自己可以有效的與使用者溝通以釐清使用者的需求，並學會引導的方式讓使用者對於每個需求提出更詳細的說明。我認為除了技術能力外，溝通的能力對於未來的職業發展也是有很大的幫助。
- 對於技術債 會怎麼處理
  - 針對這個問題，我主要會分成短期及中長期兩種解決方法。短期的部分，主要我會在開發功能的同時針對程式碼進行重構及調教。例如，在開發金流系同界接新的支付渠道時，我就將原本的業務邏輯重新整理過一遍並且將共用的部分盡可能的抽離出來確保邏輯的一致性及完整性。同時也針對一些查詢的效能進行調教，像是針對資料庫的索引或是一些SQL Script進行優化。但是我不會馬上將所有的功能都接到新的邏輯中，而是針對新開發的功能先進行對接以確定邏輯及資料正確。後續在專案的空檔在分批將舊的功能進行轉移，透過這樣的方式最小化影響線上的系統並且能夠最大化的掌握異動的內容。
  - 中長期的部分，我會重新建立一個新的功能流程並且將原有的流程重新實現。透過這樣的方式可以重新檢視整個業務邏輯並且能夠有效的將程式碼進行分層式的處理降低系統的耦合度並且適時的可以導入新的技術或新的寫法來提升系統並且不會影響到線上的功能。等到調整完畢後進行完整的回歸測試沒有問題時，在將接口轉移到新的功能去確保對線上系統的影響最小化。因為公司的開發步調比較快速，很難有完整的時間可以進行重構及調整，透過這樣逐步轉移的方式讓所有的異動範圍都在自己的掌控中。
- 當主管指派給你一個任務，你評估後認為不可行你會怎麼處理
  - 針對這個問題，如果是執行面上面有困難 例如與現有的架構有衝突或是跟原本的流程有牴觸的話。我會自己先進行完整的評估及找尋可行的替代方案，例如架構面的話可以如何調整來解決這項任務或是說流程面可以如何重新設計。舉例來說，這次公司希望我們可以導入一個訂閱制的交易流程在原有的系統中，但是我重新檢視我們的架構及流程後發現沒有辦法再現有的系統中實現。因此，我將原本的系統架構重新調整並且重新規畫了一個系統流程。然後將影響的範圍及可能的問題跟主管進行溝通及討論後，決定以我的方式重新規畫整個系統流程並再不影響現有的交易模組下進行結構的調整。
  - 如果是時程上來不及的話，我會將整個任務拆分成細部的功能開發項目。透過這樣的方式我可以更精準的評估每一個功能開發的時間後與主管進行討論，主要是看能不能重新規劃開發時程或是我們可以將每個功能建立優先順序後，針對一些比較次要的功能進行刪除或延後開發。例如，我們這次再開發店商平台的時候，因為訂單模組涵蓋的範圍無法再預計的時間完成 因此，我就將整個模組要開發的功能清單列出後，與主管討論功能的重要性並針對次要功能進行延後開發透過這樣的方式來有效的壓縮開發時程。如果經過這樣的篩選還無法在期限內完成的畫，我會再進一步跟主管討論是不是可以增加開發人力或將我手上的其他任務往後安排。如果這樣的調整還是無法順利在期限內完成的畫，通常我會利用下班時間繼續開發 希望可以協助公司完成每個任務。
- .你對你自己的評價是什麼
  - 
- 主管對你自己的評價是什麼
